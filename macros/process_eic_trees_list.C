...
// Added listFileName and maxFiles arguments with defaults    ep_10x100_q10to100/ep_10x100_q10to100.list
void process_eic_trees_list(const char* listFileName = "inputfiles/ep_18x275_minQ2_100.txt", TString outFileName = "", int maxFiles = 100) {
    // -----------------------------------------------------------------------------
    // 1. Read Text File and Chain ROOT Files
    // -----------------------------------------------------------------------------
    TChain* chain = new TChain("events");
    
    std::ifstream fileList(listFileName);
    if (!fileList.is_open()) {
        std::cerr << "Error: Could not open text file: " << listFileName << std::endl;
        return;
    }

    std::string line;
    int fileCount = 0;
    std::cout << "Adding up to " << maxFiles << " files to TChain..." << std::endl;
    while (std::getline(fileList, line)) {
        ...
        // Break if we hit the maximum number of files
        if (fileCount >= maxFiles) break;
    }
    fileList.close();

    if (fileCount == 0 || chain->GetEntries() == 0) {
        std::cerr << "Error: No files added to the chain or trees are empty!" << std::endl;
        return;
    }
    
    std::cout << "Successfully chained " << fileCount << " files." << std::endl;
    std::cout << "Total events to process: " << chain->GetEntries() << std::endl;
    ...
    outFile->Close();
    // No need to explicitly close the chain, ROOT handles cleanup
